    ...
    <div class="col-md-8 grid-margin stretch-card">
        <div class="card">
            <div class="card-header">
                <h6 class="fw-normal">Editar un <strong>Administrador</strong></h6>
            </div>
            <div class="card-body">
                <form action="{{route('admin.update', $User)}}" id="formDropzone" method="POST" enctype="multipart/form-data" novalidate>
                    @method('PUT')
                    @csrf
                    <div class="row">
                        <div class="col">
                    ...
                    <div class="row">
                        <div class="col">
                            <x-input-text title="Nombre" id="name" value="{{$User->name}}" />
                            <x-input-text title="Apellido" id="lastname" value="{{$User->lastname}}" />
                            <x-input-text title="Correo" id="email" value="{{$User->email}}" />
                            <x-input-text title="Celular" id="celular" value="{{$User->celular}}" />
                            <div class="alert alert-info" role="warning">
                                Solo en caso de cambiar la contraseña ingrese una nueva
                            </div>
                        ...
                        <div class="col bg-gris border rounded p-3">
                            <div class="form-group mb-4">
                                ...
                                <p class="text-center mb-3">
                                    <img class="rounded-circle border" width="140" id="previewImage" src="{{asset($User->getAvatarUrl('avatar'))}}" alt="Preview">
                                </p>
                                
                                <input class="form-control" type="file" id="formFile" name="profile_image" accept="image/*">
                            </div>
                            <div class="alert alert-warning" role="alert">
                                Se recomienda que las imagenes deben ser de 1:1 ejemplo (150x150) en adelante con un formato de jpg, png, jpeg, webp no mayor a 2MB
                            </div>
                        </div>
                    </div>
                </form>
